...
def nettoyer_prix(prix):
    if isinstance(prix, str):  
        # Supprimer les espaces, caractères invisibles et l'unité 'DH'
        prix = prix.replace('\u202f', '').replace(' ', '').replace('DH', '').strip()
        try:
            return float(prix) if prix else None
        except ValueError:
            return None  # Retourne None si la conversion échoue
    return None
print(nettoyer_prix('2000000DH'))  # Devrait afficher : 2000000.0
...
def detecter_et_plot_zscore(data: pd.DataFrame, colone: str):
    ...
    #plt.xlabel('Index')[df_imputed['type_de_vente']=='a louer']['new_price'].max()
    plt.ylabel(colone)
